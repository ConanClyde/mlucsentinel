/**
 * Real-time Stakeholders Updates Module
 * 
 * This module handles real-time updates for the Stakeholders table
 * using Laravel Echo and Reverb WebSocket server.
 */

class StakeholdersRealtime {
    constructor() {
    ...
    init(stakeholders = []) {
        ...
        this.tableBody = document.getElementById('stakeholdersTableBody');
    ...
    setupEcho() {
        ...
        try {
            // Subscribe to the stakeholders channel
            this.channel = window.Echo.channel('stakeholders');

            // Listen for stakeholder.updated events
            this.channel.listen('.stakeholder.updated', (event) => {
                console.log('Received broadcast event:', event);
                this.handleStakeholderUpdate(event);
            });

            // Handle connection success
            this.channel.subscribed(() => {
                console.log('Successfully subscribed to Stakeholder channel');
    ...
    /**
     * Handle Stakeholder update events
     */
    handleStakeholderUpdate(event) {
        const { stakeholder, action, editor } = event;
        ...
        switch (action) {
            ...
            case 'updated':
                ...
                if (editor && editor !== 'self' && !this.isCurrentUserAction(stakeholder.id)) {
                    this.showBrowserNotification(
                        'Stakeholder Updated',
                        `${editor} updated Stakeholder ${stakeholder.user.first_name} ${stakeholder.user.last_name}`,
                        stakeholder.id,
                        'updated'
                    );
                }
                break;
            case 'deleted':
                this.removeStakeholder(stakeholder);
                if (editor && !this.isCurrentUserAction(stakeholder.id)) {
                    this.showBrowserNotification(
                        'Stakeholder Removed',
                        `${editor} removed ${stakeholder.user.first_name} ${stakeholder.user.last_name}`,
                        null,
                        'deleted'
                    );
                }
    ...
    handleVehicleUpdate(event) {
        ...
        // Update the view modal if it's open and showing this stakeholder's vehicles
    ...
    updateViewModalVehicles(vehicle, action) {
        ...
        if (action === 'deleted' && vehicleItem) {
            ...
            // This would need to fetch the updated stakeholder data or reload the modal
     ...
     */
    isCurrentUserAction(StakeholderId) {
        if (!window._recentActions) {
            window._recentActions = new Map();
        }
        
        const now = Date.now();
        const recent = window._recentActions.get(StakeholderId);
        
        if (recent && (now - recent) < 2000) {
            window._recentActions.delete(StakeholderId);
            return true;
        }
     ...
     */
    markUserAction(StakeholderId) {
        if (!window._recentActions) {
            window._recentActions = new Map();
        }
        window._recentActions.set(StakeholderId, Date.now());
    }

    /**
     * Add a new Stakeholder row to the table
     */
    addStakeholder(stakeholder) {
        const existingRow = this.tableBody.querySelector(`tr[data-id="${stakeholder.id}"]`);
        ...
        this.stakeholders.unshift(stakeholder);
        const newRow = this.createStakeholderRow(stakeholder);
    ...
    /**
     * Update an existing Stakeholder row
     */
    updateStakeholder(stakeholder) {
        const existingRow = this.tableBody.querySelector(`tr[data-id="${stakeholder.id}"]`);
        
        if (!existingRow) {
            console.log('Stakeholder not found in table, adding instead');
            this.addStakeholder(stakeholder);
            return;
        }

        const index = this.stakeholders.findIndex(s => s.id === stakeholder.id);
        if (index !== -1) {
    ...
    /**
     * Remove a Stakeholder row from the table
     */
    removeStakeholder(stakeholder) {
        const rowToRemove = this.tableBody.querySelector(`tr[data-id="${stakeholder.id}"]`);
        
        if (!rowToRemove) {
            console.log('Stakeholder not found in table');
            return;
        }
    ...
    /**
     * Create a Stakeholder table row
     */
    createStakeholderRow(stakeholder) {
        const row = document.createElement('tr');
        row.className = 'border-b border-[#e3e3e0] dark:border-[#3E3E3A] hover:bg-gray-50 dark:hover:bg-[#161615] transition-colors';
        row.setAttribute('data-id', stakeholder.id);

        const statusClass = stakeholder.user.is_active
            ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200'
            : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200';

        const statusText = stakeholder.user.is_active ? 'Active' : 'Inactive';
        ...
        const initials = `${stakeholder.user.first_name.charAt(0)}`.toUpperCase();
        const avatarColor = this.getAvatarColor(stakeholder.user.first_name + stakeholder.user.last_name);
        ...
        row.innerHTML = `
            <td class="py-2 px-3">
                <div class="flex items-center">
                    ...
                    <div>
                        <div class="text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">${this.escapeHtml(stakeholder.user.first_name)} ${this.escapeHtml(stakeholder.user.last_name)}</div>
                    </div>
                </div>
            </td>
            <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">${this.escapeHtml(stakeholder.user.email)}</td>
            <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">${this.escapeHtml(stakeholder.type_name || 'N/A')}</td>
            <td class="py-2 px-3">
     ...
     */
    showBrowserNotification(title, message, StakeholderId = null, action = null) {
        if (Notification.permission === 'default') {
            Notification.requestPermission().then(permission => {
                if (permission === 'granted') {
                    this.displayNotification(title, message, StakeholderId, action);
                }
            });
     ...
     */
    displayNotification(title, message, StakeholderId = null, action = null) {
        const notification = new Notification(title, {
            body: message,
            icon: '/favicon.ico',
            badge: '/favicon.ico',
            tag: 'Stakeholder-update',
        ...
        notification.onclick = function() {
            ...
            if (StakeholderId && action === 'updated') {
                window.location.href = `/users/stakeholders?view=${StakeholderId}`;
            } else {
                window.location.href = '/users/stakeholders';
            }
        };
    }
    ...
    disconnect() {
        if (this.channel) {
            window.Echo.leave('stakeholders');
            this.channel = null;
            this.updateConnectionStatus('disconnected');
            console.log('Disconnected from stakeholders channel');
        }
    }
}

// Export for use in blade templates
window.StakeholdersRealtime = StakeholdersRealtime;
