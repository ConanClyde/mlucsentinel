/**
 * Real-time Staff Updates Module
 * 
 * This module handles real-time updates for the staff table
...
class StaffRealtime {
    ...
    setupEcho() {
        ...
        try {
            ...
            this.channel = window.Echo.channel('staff');

            // Listen for staff.updated events
            this.channel.listen('.staff.updated', (event) => {
                console.log('Received broadcast event:', event);
                this.handleStaffUpdate(event);
            });

            // Handle connection success
            this.channel.subscribed(() => {
                console.log('Successfully subscribed to staff channel');
    ...
    /**
     * Handle staff update events
     */
    handleStaffUpdate(event) {
        const { staff, action, editor } = event;
        ...
        switch (action) {
            ...
            case 'updated':
                ...
                if (editor && editor !== 'self' && !this.isCurrentUserAction(staff.id)) {
                    this.showBrowserNotification(
                        'Staff Updated',
                        `${editor} updated Staff ${staff.user.first_name} ${staff.user.last_name}`,
                        staff.id,
                        'updated'
                    );
                }
            ...
            case 'deleted':
                ...
                if (editor && !this.isCurrentUserAction(staff.id)) {
                    this.showBrowserNotification(
                        'Staff Removed',
                        `${editor} removed ${staff.user.first_name} ${staff.user.last_name}`,
                        null,
                        'deleted'
                    );
                }
    ...
    handleVehicleUpdate(event) {
        ...
        // Update the view modal if it's open and showing this staff's vehicles
    ...
    updateViewModalVehicles(vehicle, action) {
        ...
        if (action === 'deleted' && vehicleItem) {
            ...
            // This would need to fetch the updated staff data or reload the modal
     ...
     */
    isCurrentUserAction(staffId) {
        if (!window._recentActions) {
            window._recentActions = new Map();
        }
        
        const now = Date.now();
        const recent = window._recentActions.get(staffId);
        
        if (recent && (now - recent) < 2000) {
            window._recentActions.delete(staffId);
            return true;
        }
     ...
     */
    markUserAction(staffId) {
        if (!window._recentActions) {
            window._recentActions = new Map();
        }
        window._recentActions.set(staffId, Date.now());
    }
     ...
     */
    addStaff(staff) {
        const existingRow = this.tableBody.querySelector(`tr[data-id="${staff.id}"]`);
        ...
        const newRow = this.createStaffRow(staff);
    ...
    /**
     * Update an existing staff row
     */
    updateStaff(staff) {
        const existingRow = this.tableBody.querySelector(`tr[data-id="${staff.id}"]`);
        
        if (!existingRow) {
            console.log('Staff not found in table, adding instead');
            this.addStaff(staff);
            return;
        }
    ...
    /**
     * Remove a staff row from the table
     */
    removeStaff(staff) {
        const rowToRemove = this.tableBody.querySelector(`tr[data-id="${staff.id}"]`);
        
        if (!rowToRemove) {
            console.log('Staff not found in table');
            return;
        }
    ...
    createStaffRow(staff) {
        ...
        const statusClass = staff.user.is_active
            ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200'
            : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200';

        const statusText = staff.user.is_active ? 'Active' : 'Inactive';
        const createdDate = new Date(staff.created_at).toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric'
        });

        const initials = `${staff.user.first_name.charAt(0)}`.toUpperCase();
        const avatarColor = this.getAvatarColor(staff.user.first_name + staff.user.last_name);
        ...
        row.innerHTML = `
            <td class="py-2 px-3">
                <div class="flex items-center">
                    ...
                    <div>
                        <div class="text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">${this.escapeHtml(staff.user.first_name)} ${this.escapeHtml(staff.user.last_name)}</div>
                    </div>
                </div>
            </td>
            <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">${this.escapeHtml(staff.user.email)}</td>
            <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">${this.escapeHtml(staff.staff_id)}</td>
            <td class="py-2 px-3">
            ...
            <td class="py-2 px-3">
                <div class="flex items-center justify-center gap-2">
                    <button onclick="viewStaff(${staff.id})" class="btn-view" title="View">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M10 12a2 2 0 100-4 2 2 0 000 4z"></path>
                            <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd"></path>
                        </svg>
                    </button>
                    <button onclick="openEditModal(${staff.id})" class="btn-edit" title="Edit">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                        </svg>
                    </button>
                    <button onclick="deleteStaff(${staff.id})" class="btn-delete" title="Delete">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
    ...
    /**
     * Show empty state when no staff exist
     */
    showEmptyState() {
     ...
     */
    showBrowserNotification(title, message, staffId = null, action = null) {
        if (Notification.permission === 'default') {
            Notification.requestPermission().then(permission => {
                if (permission === 'granted') {
                    this.displayNotification(title, message, staffId, action);
                }
            });
     ...
     */
    displayNotification(title, message, staffId = null, action = null) {
        ...
        notification.onclick = function() {
            ...
            if (staffId && action === 'updated') {
                window.location.href = `/users/staff?view=${staffId}`;
            } else {
    ...
    disconnect() {
        if (this.channel) {
            window.Echo.leave('staff');
            this.channel = null;
            this.updateConnectionStatus('disconnected');
            console.log('Disconnected from staff channel');
        }
    }
}

// Export for use in blade templates
window.StaffRealtime = StaffRealtime;
