...
<div class="space-y-6">
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Patrol Statistics (Last 24 Hours)</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A]">Total Check-ins</p>
                        <p class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">{{ number_format($patrolStats['total_checkins']) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A]">Active Guards</p>
                        <p class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">{{ number_format($patrolStats['unique_guards']) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A]">Locations Covered</p>
                        <p class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">{{ number_format($patrolStats['unique_locations']) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A]">Most Visited</p>
                        <p class="text-lg font-bold text-[#1b1b18] dark:text-[#EDEDEC] truncate">
                            {{ $patrolStats['most_visited_location']?->mapLocation?->short_code ?? 'N/A' }}
                        </p>
                        <p class="text-xs text-[#706f6c] dark:text-[#A1A09A]">{{ number_format($patrolStats['most_visited_location']?->visit_count ?? 0) }} visits</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h3 class="text-lg font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Campus Violation Heatmap</h3>
                    <p class="text-sm text-[#706f6c] dark:text-[#A1A09A] mt-1">Color intensity shows where violations occur most frequently</p>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex space-x-2">
                        <button id="heatmap-zoom-in" class="flex items-center justify-center w-8 h-8 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                            <span class="text-lg font-bold leading-none">+</span>
                        </button>
                        <button id="heatmap-zoom-out" class="flex items-center justify-center w-8 h-8 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                            <span class="text-lg font-bold leading-none">−</span>
                        </button>
                        <button id="heatmap-reset" class="flex items-center justify-center w-8 h-8 border border-gray-300 dark:border-gray-600 text-[#1b1b18] dark:text-[#EDEDEC] rounded hover:border-gray-400 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="relative overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-900" id="heatmap-container">
                <div id="heatmap-wrapper" class="relative w-full origin-center transition-transform duration-300 ease-out">
                    ...
                    <!-- Heatmap Overlay -->
                    <svg class="absolute inset-0 w-full h-full pointer-events-none" id="heatmap-svg" viewBox="0 0 100 100" preserveAspectRatio="none">
                        <defs>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeHeatmapDimensions() {
        ...
        // Set container height based on image aspect ratio (same as campus-map)
        if (img && img.complete && img.naturalHeight > 0) {
            heatmapAspectRatio = img.naturalHeight / img.naturalWidth;
            const containerWidth = container.offsetWidth;
            const newHeight = containerWidth * heatmapAspectRatio;
    ...
    function renderHeatmapLocations() {
        ...
        mapLocations.forEach(location => {
            ...
            // Draw polygon (transparent, shows on hover)
            ...
            // Create label (will show on hover, hidden for parking)
            let label = null;
            if (location.center_x && location.center_y && !isParking) {
                ...
                label.setAttribute('opacity', '0'); // Hidden by default
            ...
            g.addEventListener('mouseenter', function(e) {
                // Show polygon with location color
                polygon.setAttribute('fill', location.color + '20'); // 12% opacity
                polygon.setAttribute('stroke', location.color + '60'); // 38% opacity
                ...
                let tooltipContent = `
                    ...
                    ${location.short_code ? `<div class="text-xs text-gray-400 dark:text-gray-600 mt-1">Code: ${location.short_code}</div>` : ''}
                    <div class="text-xs text-gray-400 dark:text-gray-600 mt-1">Type: ${location.type.name}</div>
                `;
                
                if (location.description) {
                    tooltipContent += `<div class="text-xs text-gray-300 dark:text-gray-700 mt-2 max-w-xs">${location.description}</div>`;
                }
    ...
    if (heatmapImg.complete) {
        ...
        heatmapImg.addEventListener('load', initializeHeatmapDimensions);
    }
    // Add error handler
