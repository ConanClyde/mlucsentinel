...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-4 md:p-6">
        <div class="flex flex-col lg:flex-row gap-3 md:gap-4">
            ...
            <div class="flex-[2]">
                ...
                <input type="text" id="search-input" class="form-input w-full" placeholder="Search by name, email, or Security ID...">
            </div>
    ...
    <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-4 md:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 md:mb-6">
            <h3 class="text-base md:text-lg font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Security List</h3>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A]">
                        @if(Auth::user()->hasAnyPrivilege(['edit_security', 'delete_security']))
                        ...
                        <th class="text-left py-2 px-3 text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Security ID</th>
                ...
                <tbody id="securityTableBody">
                    @forelse($security as $securityMember)
                    <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A] hover:bg-gray-50 dark:hover:bg-[#161615]" data-id="{{ $securityMember->user_id }}">
                        @if(Auth::user()->hasAnyPrivilege(['edit_security', 'delete_security']))
                        <td class="text-center py-2 px-3">
                            <input type="checkbox" class="row-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" value="{{ $securityMember->user_id }}" onchange="updateBulkActions()">
                        </td>
                        @endif
                        <td class="py-2 px-3">
                            <div class="flex items-center">
                                @php
                                    ...
                                    $firstLetter = strtoupper(substr($securityMember->user->first_name ?? 'U', 0, 1));
                                ...
                                <div>
                                    <div class="text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">{{ $securityMember->user->first_name ?? '' }} {{ $securityMember->user->last_name ?? '' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $securityMember->user->email ?? 'N/A' }}</td>
                        <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $securityMember->security_id ?? 'N/A' }}</td>
                        <td class="py-2 px-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ ($securityMember->user->is_active ?? false) ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' }}">
                                {{ ($securityMember->user->is_active ?? false) ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $securityMember->created_at->format('M d, Y') }}</td>
                        <td class="py-2 px-3">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="viewSecurity({{ $securityMember->id }})" class="btn-view" title="View">
                                ...
                                @if(Auth::user()->isGlobalAdministrator() || Auth::user()->isSecurityAdmin())
                                    <button onclick="openEditModal({{ $securityMember->id }})" class="btn-edit" title="Edit">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <button onclick="deleteSecurity({{ $securityMember->id }})" class="btn-delete" title="Delete">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ Auth::user()->hasAnyPrivilege(['edit_security', 'delete_security']) ? '7' : '6' }}" class="py-8 px-4 text-center text-[#706f6c] dark:text-[#A1A09A]">
                            No security found.
                        </td>
                    </tr>
                    @endforelse
        ...
        <div id="pagination-controls" class="flex items-center justify-between mt-6">
            <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">
                Showing <span id="showing-start">1</span>-<span id="showing-end">10</span> of <span id="total-count">0</span> security
            </p>
...
<div id="viewModal" class="modal-backdrop hidden" onclick="if(event.target === this) closeViewModal()">
    <div class="modal-container-wide">
        <div class="modal-header">
            <h2 class="modal-title">Security Details</h2>
        </div>
...
<div id="editModal" class="modal-backdrop hidden" onclick="if(event.target === this) closeEditModal()">
    <div class="modal-container-wide">
        <div class="modal-header">
            <h2 class="modal-title">Edit Security</h2>
        </div>
        <form id="editForm">
            <div class="modal-body max-h-[70vh] overflow-y-auto">
                <input type="hidden" id="edit_security_id">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="form-group">
                        <label class="form-label">Security ID <span class="text-red-500">*</span></label>
                        <input type="text" id="edit_security_id_input" class="form-input" required>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    <h3 class="text-lg font-semibold text-[#1b1b18] dark:text-[#EDEDEC] mb-3">Change Password (Optional)</h3>
                    <p class="text-sm text-[#706f6c] dark:text-[#A1A09A] mb-4">Leave blank to keep the current password</p>
                    
                    <div class="space-y-4">
                        <div class="form-group">
                            <label class="form-label">New Password</label>
                            <div class="relative">
                                <input type="password" id="edit_password" name="password" class="form-input pr-10" placeholder="Enter new password">
                                <button type="button" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" onclick="toggleEditPasswordVisibility('edit_password')">
                                    <x-heroicon-c-eye id="edit_password-eye-icon" class="w-5 h-5" />
                                    <x-heroicon-c-eye-slash id="edit_password-eye-off-icon" class="w-5 h-5 hidden" />
                                </button>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="relative">
                                <input type="password" id="edit_password_confirmation" name="password_confirmation" class="form-input pr-10" placeholder="Confirm new password">
                                <button type="button" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" onclick="toggleEditPasswordVisibility('edit_password_confirmation')">
                                    <x-heroicon-c-eye id="edit_password_confirmation-eye-icon" class="w-5 h-5" />
                                    <x-heroicon-c-eye-slash id="edit_password_confirmation-eye-off-icon" class="w-5 h-5 hidden" />
                                </button>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
<script>
// Initialize real-time updates using the securityRealtime module
let realtimeManager;
let security = @json($security->items() ?? []);
let vehicleTypes = @json($vehicleTypes ?? []);
let deleteSecurityId = null;
window.currentUserId = {{ auth()->id() }};


// Process security data to add proper image paths
security = security.map(function(securityMember) {
    return {
        ...securityMember,
        license_image: securityMember.license_image ? (
            securityMember.license_image.startsWith('/storage/') || securityMember.license_image.startsWith('http') 
                ? securityMember.license_image 
                : '/storage/' + securityMember.license_image
        ) : null,
        vehicles: securityMember.vehicles ? securityMember.vehicles.map(function(vehicle) {
            return {
                ...vehicle,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.SecurityRealtime) {
        realtimeManager = new window.SecurityRealtime();
        realtimeManager.init(security);
        
        // Update local security array when real-time updates occur
        window.Echo.channel('security').listen('.security.updated', (event) => {
            const index = security.findIndex(s => s.id === event.security.id);
            
            // Process license_image path
            const processedsecurityMember = {
                ...event.security,
                license_image: event.security.license_image && 
                    !event.security.license_image.startsWith('/storage/') && 
                    !event.security.license_image.startsWith('http') 
                    ? '/storage/' + event.security.license_image 
                    : event.security.license_image,
                vehicles: event.security.vehicles ? event.security.vehicles.map(function(vehicle) {
            ...
            if (index !== -1) {
                security[index] = processedsecurityMember;
                
                // If view modal is open for this security, refresh it
                const viewModal = document.getElementById('viewModal');
                if (!viewModal.classList.contains('hidden')) {
                    viewSecurity(processedsecurityMember.id);
                }
                
                // If edit modal is open for this security, refresh the existing vehicles display
                const editModal = document.getElementById('editModal');
                if (!editModal.classList.contains('hidden')) {
                    const editsecurityId = document.getElementById('edit_security_id').value;
                    if (editsecurityId == processedsecurityMember.id) {
                        currentEditsecurityVehicles = processedsecurityMember.vehicles || [];
                        displayExistingVehicles();
                    }
                }
            } else if (event.action === 'created') {
                security.unshift(processedsecurityMember);
            }
        });
    } else {
        console.error('securityRealtime module not loaded');
    }
    ...
    function applyPagination() {
        const rows = document.querySelectorAll('#securityTableBody tr');
        let visibleCount = 0;
        let totalFiltered = 0;
        ...
        rows.forEach((row) => {
            ...
            const securityMemberId = row.querySelector('td:nth-child(3)')?.textContent.toLowerCase() || '';
            ...
            const matchesSearch = name.includes(searchTerm) || email.includes(searchTerm) || securityMemberId.includes(searchTerm);
...
function viewSecurity(id) {
    const securityMember = security.find(s => s.id === id);
    ...
    let vehiclesHtml = '';
    if (securityMember.vehicles && securityMember.vehicles.length > 0) {
        ...
        securityMember.vehicles.forEach((vehicle, index) => {
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${securityMember.user.first_name} ${securityMember.user.last_name}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Email</h3>
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${securityMember.user.email}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Security ID</h3>
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${securityMember.security_id}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">License Number</h3>
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${securityMember.license_no || 'N/A'}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Status</h3>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${securityMember.user.is_active ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200'}">
                        ${securityMember.user.is_active ? 'Active' : 'Inactive'}
                    </span>
                </div>
            </div>
            ${securityMember.license_image ? `
            <div>
                ...
                <div class="image-container">
                    <img src="${securityMember.license_image}" alt="License" class="license-image">
                </div>
            </div>
            ` : ''}
            ${vehiclesHtml}
...
function deleteSecurity(id) {
    ...
    document.getElementById('deleteModalMessage').textContent = 
        `Are you sure you want to delete ${securityMember.user.first_name} ${securityMember.user.last_name}? This action cannot be undone.`;
...
function confirmDeleteSecurity() {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.message || 'Failed to Delete Security member');
        }
    })
    .catch(error => {
...
let currentEditsecurityVehicles = [];
...
function displayExistingVehicles() {
    ...
    if (!currentEditsecurityVehicles || currentEditsecurityVehicles.length === 0) {
    ...
    currentEditsecurityVehicles.forEach((vehicle, index) => {
        const vehicleType = vehicleTypes.find(vt => vt.id === vehicle.type_id);
        const typeName = vehicleType ? vehicleType.name : 'Unknown';
...
function addEditVehicle() {
    ...
    const activeVehicles = currentEditsecurityVehicles.filter(v => !vehiclesToDelete.includes(v.id)).length;
    const totalVehicles = activeVehicles + editVehicleCount;
    
    if (totalVehicles >= 3) {
        alert('Maximum 3 vehicles allowed per securityMember');
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editForm) {
        editForm.addEventListener('submit', function(e) {
            ...
            const id = document.getElementById('edit_security_id').value;
            ...
            if (hasNewLicenseImage) {
                ...
                formData.append('security_id', document.getElementById('edit_security_id_input').value);
                ...
                // Append password fields only if they have values
                ...
                fetchPromise = fetch(`/users/security/${id}`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                ...
                const data = {
                    ...
                    security_id: document.getElementById('edit_security_id_input').value,
                ...
                // Add password fields only if they have values
                const password = document.getElementById('edit_password').value;
                const passwordConfirmation = document.getElementById('edit_password_confirmation').value;
                if (password) {
            ...
            fetchPromise
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert(data.message || 'Failed to update security member');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
...
function exportToCSV() {
    const visiblesecurity = security.filter(securityMember => {
        const row = document.querySelector(`tr[data-id="${securityMember.id}"]`);
        return row && row.style.display !== 'none';
    });

    if (visiblesecurity.length === 0) {
        alert('No security to export');
        return;
    }

    const headers = ['Name', 'Email', 'Security ID', 'Status', 'Created Date'];
    const rows = visiblesecurity.map(securityMember => {
        const name = `${securityMember.user.first_name} ${securityMember.user.last_name}`;
        const email = securityMember.user.email;
        const securityId = securityMember.security_id;
        const status = securityMember.user.is_active ? 'Active' : 'Inactive';
        const createdDate = new Date(securityMember.created_at).toLocaleDateString('en-US', { 
            year: 'numeric', month: 'long', day: 'numeric' 
        });
        
        return [name, email, securityId, status, createdDate].map(field => {
    ...
    link.setAttribute('download', `security_${new Date().toISOString().split('T')[0]}.csv`);
...
function openEditModal(id) {
    ...
    document.getElementById('edit_security_id').value = securityMember.id;
    document.getElementById('edit_first_name').value = securityMember.user.first_name;
    document.getElementById('edit_last_name').value = securityMember.user.last_name;
    document.getElementById('edit_email').value = securityMember.user.email;
    document.getElementById('edit_security_id_input').value = securityMember.security_id;
    document.getElementById('edit_license_no').value = securityMember.license_no;
    document.getElementById('edit_is_active').value = securityMember.user.is_active ? '1' : '0';
    
    // Display current license image if exists
    if (securityMember.license_image) {
        document.getElementById('editCurrentLicenseImage').classList.remove('hidden');
        document.getElementById('editCurrentLicenseImageSrc').src = securityMember.license_image;
    } else {
    ...
    currentEditsecurityVehicles = securityMember.vehicles || [];
...
window.toggleEditPasswordVisibility = function(inputId) {
    const input = document.getElementById(inputId);
    const eyeIcon = document.getElementById(`${inputId}-eye-icon`);
    const eyeOffIcon = document.getElementById(`${inputId}-eye-off-icon`);
...
window.viewSecurity = viewSecurity;
...
window.displayExistingVehicles = displayExistingVehicles;
...
function bulkActivate() {
    ...
    if (selected.length === 0) {
        alert('Please select at least one security member');
        return;
    }
    
    const count = selected.length;
    const message = count === 1 
        ? 'Are you sure you want to activate 1 security member?'
        : `Are you sure you want to activate ${count} security members?`;
...
function executeBulkActivate() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert(data.message || 'Error activating security members');
        }
    })
    .catch(error => {
        bulkBar.style.opacity = '1';
        bulkBar.style.pointerEvents = 'auto';
        console.error('Error:', error);
        alert('Error activating security members');
    });
}
...
function bulkDeactivate() {
    ...
    const count = selected.length;
    const message = count === 1 
        ? 'Are you sure you want to deactivate 1 security member?'
        : `Are you sure you want to deactivate ${count} security members?`;
...
function executeBulkDeactivate() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert(data.message || 'Error deactivating security members');
        }
    })
    .catch(error => {
        bulkBar.style.opacity = '1';
        bulkBar.style.pointerEvents = 'auto';
        console.error('Error:', error);
        alert('Error deactivating security members');
    });
}
...
function confirmBulkDelete() {
    ...
    const count = selected.length;
    const message = count === 1 
        ? 'Are you sure you want to delete 1 security member? This action cannot be undone.'
        : `Are you sure you want to delete ${count} security members? This action cannot be undone.`;
...
function executeBulkDelete() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert(data.message || 'Error deleting security members');
        }
    })
    .catch(error => {
        bulkBar.style.opacity = '1';
        bulkBar.style.pointerEvents = 'auto';
        console.error('Error:', error);
        alert('Error deleting security members');
    });
}
