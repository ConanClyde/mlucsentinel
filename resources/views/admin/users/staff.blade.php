...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-4 md:p-6">
        <div class="flex flex-col lg:flex-row gap-3 md:gap-4">
            ...
            <div class="flex-[2]">
                ...
                <input type="text" id="search-input" class="form-input w-full" placeholder="Search by name, email, or staff ID...">
            </div>
    ...
    <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-4 md:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 md:mb-6">
            <h3 class="text-base md:text-lg font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Staff List</h3>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A]">
                        @if(Auth::user()->hasAnyPrivilege(['edit_staff', 'delete_staff']))
                        ...
                        <th class="text-left py-2 px-3 text-xs font-medium text-[#706f6c] dark:text-[#A1A09A] uppercase tracking-wider">Staff ID</th>
                ...
                <tbody id="staffTableBody">
                    ...
                    <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A] hover:bg-gray-50 dark:hover:bg-[#161615]" data-id="{{ $staffMember->user_id }}">
                        @if(Auth::user()->hasAnyPrivilege(['edit_staff', 'delete_staff']))
                        <td class="text-center py-2 px-3">
                            <input type="checkbox" class="row-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" value="{{ $staffMember->user_id }}" onchange="updateBulkActions()">
                        </td>
                        @endif
                        <td class="py-2 px-3">
                            <div class="flex items-center">
                                @php
                                    ...
                                    $firstLetter = strtoupper(substr($staffMember->user->first_name ?? 'U', 0, 1));
                                ...
                                <div>
                                    <div class="text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">{{ $staffMember->user->first_name ?? '' }} {{ $staffMember->user->last_name ?? '' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $staffMember->user->email ?? 'N/A' }}</td>
                        <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $staffMember->staff_id ?? 'N/A' }}</td>
                        <td class="py-2 px-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ ($staffMember->user->is_active ?? false) ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' }}">
                                {{ ($staffMember->user->is_active ?? false) ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="py-2 px-3 text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $staffMember->created_at->format('M d, Y') }}</td>
                        <td class="py-2 px-3">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="viewStaff({{ $staffMember->id }})" class="btn-view" title="View">
                                ...
                                @if(Auth::user()->isGlobalAdministrator() || Auth::user()->isSecurityAdmin())
                                    <button onclick="openEditModal({{ $staffMember->id }})" class="btn-edit" title="Edit">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <button onclick="deleteStaff({{ $staffMember->id }})" class="btn-delete" title="Delete">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ Auth::user()->hasAnyPrivilege(['edit_staff', 'delete_staff']) ? '7' : '6' }}" class="py-8 px-4 text-center text-[#706f6c] dark:text-[#A1A09A]">
                            No staff found.
                        </td>
                    </tr>
                    @endforelse
        ...
        <div id="pagination-controls" class="flex items-center justify-between mt-6">
            <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">
                Showing <span id="showing-start">1</span>-<span id="showing-end">10</span> of <span id="total-count">0</span> staff
            </p>
...
<div id="deleteModal" class="modal-backdrop hidden" onclick="if(event.target === this) closeDeleteModal()">
    <div class="modal-container">
        ...
        <div class="modal-footer">
            ...
            <button onclick="confirmDeleteStaff()" class="btn btn-danger">Delete</button>
        </div>
    </div>
</div>
            ...
            <div class="modal-body max-h-[70vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="edit_staff_id_input" class="form-input" required>
                    </div>
                </div>
...
@push('scripts')
<script>
// Initialize real-time updates using the staffRealtime module
let realtimeManager;
let staff = @json($staff->items() ?? []);
let vehicleTypes = @json($vehicleTypes ?? []);
let deleteStaffId = null;
window.currentUserId = {{ auth()->id() }};


// Process staff data to add proper image paths
staff = staff.map(function(staffMember) {
    return {
        ...staffMember,
        license_image: staffMember.license_image ? (
            staffMember.license_image.startsWith('/storage/') || staffMember.license_image.startsWith('http') 
                ? staffMember.license_image 
                : '/storage/' + staffMember.license_image
        ) : null,
        vehicles: staffMember.vehicles ? staffMember.vehicles.map(function(vehicle) {
            return {
                ...vehicle,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.StaffRealtime) {
        realtimeManager = new window.StaffRealtime();
        realtimeManager.init(staff);
        
        // Update local staff array when real-time updates occur
        window.Echo.channel('staff').listen('.staff.updated', (event) => {
            const index = staff.findIndex(s => s.id === event.staff.id);
            
            // Process license_image path
            const processedStaffMember = {
                ...event.staff,
                license_image: event.staff.license_image && 
                    !event.staff.license_image.startsWith('/storage/') && 
                    !event.staff.license_image.startsWith('http') 
                    ? '/storage/' + event.staff.license_image 
                    : event.staff.license_image,
                vehicles: event.staff.vehicles ? event.staff.vehicles.map(function(vehicle) {
            ...
            if (index !== -1) {
                staff[index] = processedStaffMember;
                
                // If view modal is open for this staff, refresh it
                const viewModal = document.getElementById('viewModal');
                if (!viewModal.classList.contains('hidden')) {
                    viewStaff(processedStaffMember.id);
                }
                
                // If edit modal is open for this staff, refresh the existing vehicles display
                const editModal = document.getElementById('editModal');
                if (!editModal.classList.contains('hidden')) {
                    const editStaffId = document.getElementById('edit_staff_id').value;
                    if (editStaffId == processedStaffMember.id) {
                        currentEditStaffVehicles = processedStaffMember.vehicles || [];
                        displayExistingVehicles();
                    }
                }
            } else if (event.action === 'created') {
                staff.unshift(processedStaffMember);
            }
        });
    } else {
        console.error('StaffRealtime module not loaded');
    }
    ...
    function applyPagination() {
        ...
        rows.forEach((row) => {
            ...
            const staffMemberId = row.querySelector('td:nth-child(3)')?.textContent.toLowerCase() || '';
            ...
            const matchesSearch = name.includes(searchTerm) || email.includes(searchTerm) || staffMemberId.includes(searchTerm);
...
function viewStaff(id) {
    ...
    let vehiclesHtml = '';
    if (staffMember.vehicles && staffMember.vehicles.length > 0) {
        ...
        staffMember.vehicles.forEach((vehicle, index) => {
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${staffMember.user.first_name} ${staffMember.user.last_name}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Email</h3>
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${staffMember.user.email}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Staff ID</h3>
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${staffMember.staff_id}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">License Number</h3>
                    <p class="text-[#1b1b18] dark:text-[#EDEDEC]">${staffMember.license_no || 'N/A'}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Status</h3>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${staffMember.user.is_active ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200'}">
            ...
            <div>
                ...
                <div class="image-container">
                    <img src="${staffMember.license_image}" alt="License" class="license-image">
                </div>
            </div>
            ` : ''}
            ${vehiclesHtml}
...
function deleteStaff(id) {
    ...
    document.getElementById('deleteModalMessage').textContent = 
        `Are you sure you want to delete ${staffMember.user.first_name} ${staffMember.user.last_name}? This action cannot be undone.`;
...
function confirmDeleteStaff() {
    ...
    fetch(`/users/staff/${deleteStaffId}`, {
        method: 'DELETE',
        headers: {
...
function displayExistingVehicles() {
    ...
    if (!currentEditStaffVehicles || currentEditStaffVehicles.length === 0) {
    ...
    currentEditStaffVehicles.forEach((vehicle, index) => {
        const vehicleType = vehicleTypes.find(vt => vt.id === vehicle.type_id);
        const typeName = vehicleType ? vehicleType.name : 'Unknown';
...
function addEditVehicle() {
    ...
    const activeVehicles = currentEditStaffVehicles.filter(v => !vehiclesToDelete.includes(v.id)).length;
    const totalVehicles = activeVehicles + editVehicleCount;
    
    if (totalVehicles >= 3) {
        alert('Maximum 3 vehicles allowed per staffMember');
        return;
    }
    ...
    if (editForm) {
        editForm.addEventListener('submit', function(e) {
            ...
            if (hasNewLicenseImage) {
                ...
                formData.append('staff_id', document.getElementById('edit_staff_id_input').value);
                ...
                fetchPromise = fetch(`/users/staff/${id}`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                ...
                const data = {
                    ...
                    staff_id: document.getElementById('edit_staff_id_input').value,
...
function exportToCSV() {
    const visibleStaff = staff.filter(staffMember => {
        const row = document.querySelector(`tr[data-id="${staffMember.id}"]`);
    ...
    const headers = ['Name', 'Email', 'Staff ID', 'Status', 'Created Date'];
    const rows = visibleStaff.map(staffMember => {
        const name = `${staffMember.user.first_name} ${staffMember.user.last_name}`;
        const email = staffMember.user.email;
        const staffId = staffMember.staff_id;
        const status = staffMember.user.is_active ? 'Active' : 'Inactive';
        const createdDate = new Date(staffMember.created_at).toLocaleDateString('en-US', { 
            year: 'numeric', month: 'long', day: 'numeric' 
        });
        
        return [name, email, staffId, status, createdDate].map(field => {
...
function openEditModal(id) {
    ...
    document.getElementById('edit_staff_id').value = staffMember.id;
    document.getElementById('edit_first_name').value = staffMember.user.first_name;
    document.getElementById('edit_last_name').value = staffMember.user.last_name;
    document.getElementById('edit_email').value = staffMember.user.email;
    document.getElementById('edit_staff_id_input').value = staffMember.staff_id;
    document.getElementById('edit_license_no').value = staffMember.license_no;
    document.getElementById('edit_is_active').value = staffMember.user.is_active ? '1' : '0';
    
    // Display current license image if exists
    if (staffMember.license_image) {
        document.getElementById('editCurrentLicenseImage').classList.remove('hidden');
        document.getElementById('editCurrentLicenseImageSrc').src = staffMember.license_image;
    } else {
    ...
    currentEditStaffVehicles = staffMember.vehicles || [];
...
function bulkActivate() {
    ...
    const message = count === 1 
        ? 'Are you sure you want to activate 1 staff member?'
        : `Are you sure you want to activate ${count} staff members?`;
...
function executeBulkActivate() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert(data.message || 'Error activating staff members');
        }
    })
    .catch(error => {
        bulkBar.style.opacity = '1';
        bulkBar.style.pointerEvents = 'auto';
        console.error('Error:', error);
        alert('Error activating staff members');
    });
}
...
function bulkDeactivate() {
    ...
    const count = selected.length;
    const message = count === 1 
        ? 'Are you sure you want to deactivate 1 staff member?'
        : `Are you sure you want to deactivate ${count} staff members?`;
...
function executeBulkDeactivate() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert(data.message || 'Error deactivating staff members');
        }
    })
    .catch(error => {
        bulkBar.style.opacity = '1';
        bulkBar.style.pointerEvents = 'auto';
        console.error('Error:', error);
        alert('Error deactivating staff members');
    });
}
...
function confirmBulkDelete() {
    ...
    const count = selected.length;
    const message = count === 1 
        ? 'Are you sure you want to delete 1 staff member? This action cannot be undone.'
        : `Are you sure you want to delete ${count} staff members? This action cannot be undone.`;
...
function executeBulkDelete() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert(data.message || 'Error deleting staff members');
        }
    })
    .catch(error => {
        bulkBar.style.opacity = '1';
        bulkBar.style.pointerEvents = 'auto';
        console.error('Error:', error);
        alert('Error deleting staff members');
    });
}
