...
@section('title', 'Security Registration')
...
<div class="min-h-screen py-8">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 text-center">
            <h1 class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Security Registration</h1>
            <p class="text-[#706f6c] dark:text-[#A1A09A]">Register new Security personnel with vehicle information and account credentials</p>
        </div>
        ...
        <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6 mb-6">
            <div class="flex items-center justify-center">
                <div class="flex items-center space-x-4">
                    ...
                    <!-- Step 4 -->
                    <div class="flex items-center">
                        <div id="step-4-indicator" class="w-8 h-8 rounded-full bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-400 flex items-center justify-center text-sm font-medium">
                            4
                        </div>
                        <span id="step-4-label" class="ml-2 text-sm font-medium text-gray-500 dark:text-gray-400">Account Information</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
            <form id="securityRegistrationForm" enctype="multipart/form-data">
                @csrf
                ...
                <div id="step-1" class="step-content p-6">
                    ...
                    <div class="mt-6">
                        <label for="security_id" class="block text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC] mb-2">
                            Security ID <span class="text-red-500">*</span>
                        </label>
                        <input type="text" id="security_id" name="security_id" class="form-input" placeholder="123456" required>
                        <div id="security_id_error" class="text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                ...
                <p class="text-xs text-[#706f6c] dark:text-[#A1A09A] mt-2">
                    Maximum of 3 vehicles allowed per security member
                </p>
            </div>

                <!-- Step 4: Account Information -->
                <div id="step-4" class="step-content p-6 hidden">
                    <h3 class="text-lg font-semibold text-[#1b1b18] dark:text-[#EDEDEC] mb-6">Account Information</h3>
                    <p class="text-sm text-[#706f6c] dark:text-[#A1A09A] mb-6">Create login credentials for the security personnel to report violators</p>
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-[#e3e3e0] dark:border-[#3E3E3A] px-6 pb-6">
                    ...
                    <!-- Step 4 Buttons -->
                    <div id="step-4-buttons" class="hidden">
...
<div id="successModal" class="modal-backdrop hidden">
    <div class="modal-container">
        ...
        <div class="modal-body">
            <p>The Staff has been registered successfully and can now be assigned vehicles.</p>
        </div>
        <div class="modal-footer">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            fetch('{{ route("admin.registration.security.store") }}', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    const securityIdInput = document.getElementById('security_id');
    ...
    if (securityIdInput) {
        securityIdInput.addEventListener('input', function() {
            clearTimeout(securityIdTimeout);
            securityIdTimeout = setTimeout(validatesecurityId, 1000);
            updateButtonStates();
        });
    }
    ...
    function validatesecurityId() {
        ...
        if (!securityId) {
            showFieldError('security_id', 'Security ID is required');
            return false;
        } else if (!isValidsecurityId(securityId)) {
            showFieldError('security_id', 'Security ID must contain at least 6 digits');
            return false;
        } else {
            // Check Security ID availability
            checksecurityIdAvailability(securityId);
            clearFieldError('security_id');
            return true;
        }
    }
    ...
    function checkEmailAvailability(email) {
                fetch('{{ route("admin.registration.security.check-email") }}', {
                    method: 'POST',
                    headers: {
    ...
    function checksecurityIdAvailability(securityId) {
        fetch('{{ route("admin.registration.security.check-security-id") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        .then(data => {
            if (data.available) {
                clearFieldError('security_id');
                availabilityErrors.securityId = false;
            } else {
                showFieldError('security_id', data.message);
                availabilityErrors.securityId = true;
            }
            updateButtonStates();
        })
        .catch(error => {
            console.error('Security ID check error:', error);
            clearFieldError('security_id');
            availabilityErrors.securityId = false;
            updateButtonStates();
        });
    ...
    function checkLicenseNoAvailability(licenseNo) {
        fetch('{{ route("admin.registration.security.check-license-no") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function checkPlateNumberAvailability(plateNo, vehicleIndex) {
    fetch('{{ route("admin.registration.security.check-plate-no") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function isStep1Valid() {
    ...
    const hasNoErrors = !availabilityErrors.email && !availabilityErrors.securityId;
    
    return firstName && lastName && securityId && email && isValidEmail(email) && isValidsecurityId(securityId) && hasNoErrors;
}
...
function validateCurrentStep() {
    if (currentStep === 1) {
        ...
        if (!firstName || !lastName || !securityId || !email) {
            showErrorModal('Please fill in all required fields');
            return false;
        }
        
        if (!isValidsecurityId(securityId)) {
            showErrorModal('Security ID must contain at least 6 digits');
            return false;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addVehicleBtn.addEventListener('click', function() {
        if (vehicleCount >= 3) {
            alert('Maximum of 3 vehicles allowed per security member');
            return;
        }
...
// Validate Security ID format (minimum 6 digits)
function isValidsecurityId(securityId) {
    // Remove any non-digit characters
    const digitsOnly = securityId.replace(/\D/g, '');
