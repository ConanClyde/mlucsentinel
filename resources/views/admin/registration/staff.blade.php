...
<div class="min-h-screen py-4 md:py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
            <form id="staffRegistrationForm" enctype="multipart/form-data">
                @csrf
                ...
                <div id="step-1" class="step-content p-4 md:p-6">
                    ...
                    <div class="mt-4 md:mt-6">
                        ...
                        <input type="text" id="staff_id" name="staff_id" class="form-input" placeholder="123456" required>
                ...
                <p class="text-xs text-[#706f6c] dark:text-[#A1A09A] mt-2">
                    Maximum of 3 vehicles allowed per staff member
                </p>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            fetch('{{ route("admin.registration.staff.store") }}', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    if (staffIdInput) {
        staffIdInput.addEventListener('input', function() {
            ...
            staffIdTimeout = setTimeout(validateStaffId, 1000);
            updateButtonStates();
        });
    }
    ...
    function checkEmailAvailability(email) {
                fetch('{{ route("admin.registration.staff.check-email") }}', {
                    method: 'POST',
                    headers: {
    ...
    function checkStaffIdAvailability(staffId) {
        fetch('{{ route("admin.registration.staff.check-staff-id") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    function checkLicenseNoAvailability(licenseNo) {
        fetch('{{ route("admin.registration.staff.check-license-no") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function checkPlateNumberAvailability(plateNo, vehicleIndex) {
    fetch('{{ route("admin.registration.staff.check-plate-no") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function validateCurrentStep() {
    if (currentStep === 1) {
        ...
        if (!isValidStaffId(staffId)) {
            showErrorModal('Staff ID must contain at least 6 digits');
            return false;
        }
...
// Validate Staff ID format (minimum 6 digits)
function isValidStaffId(staffId) {
    // Remove any non-digit characters
    const digitsOnly = staffId.replace(/\D/g, '');
