...
@section('title', 'Stakeholder Registration')
...
<div class="min-h-screen py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 text-center">
            <h1 class="text-2xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">Stakeholder Registration</h1>
            <p class="text-[#706f6c] dark:text-[#A1A09A]">Register new Stakeholder users with vehicle information</p>
            </div>
        ...
        <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
            <form id="StakeholderRegistrationForm" enctype="multipart/form-data">
                @csrf
...
<div id="successModal" class="modal-backdrop hidden">
    <div class="modal-container">
        ...
        <div class="modal-body">
            <p>The Stakeholder has been registered successfully and can now be assigned vehicles.</p>
        </div>
        <div class="modal-footer">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Form submission
    const form = document.getElementById('StakeholderRegistrationForm');
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            fetch('{{ route("admin.registration.stakeholder.store") }}', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    function checkEmailAvailability(email) {
                fetch('{{ route("admin.registration.stakeholder.check-email") }}', {
                    method: 'POST',
                    headers: {
    ...
    function checkLicenseNoAvailability(licenseNo) {
        fetch('{{ route("admin.registration.stakeholder.check-license-no") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function checkPlateNumberAvailability(plateNo, vehicleIndex) {
    fetch('{{ route("admin.registration.stakeholder.check-plate-no") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function validateCurrentStep() {
    if (currentStep === 1) {
        ...
        if (!firstName || !lastName || !typeId || !email) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addVehicleBtn.addEventListener('click', function() {
        if (vehicleCount >= 3) {
            alert('Maximum of 3 vehicles allowed per Stakeholder member');
            return;
        }
...
// Validate Stakeholder ID format (minimum 6 digits)
function isValidStakeholderId(StakeholderId) {
    // Remove any non-digit characters
    const digitsOnly = StakeholderId.replace(/\D/g, '');
