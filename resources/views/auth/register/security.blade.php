...
@section('title', 'Security Registration - MLUC Sentinel')
...
<div class="min-h-screen py-4 md:py-8 flex items-center justify-center">
    <div class="max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
            <form id="securityRegistrationForm" method="POST" action="{{ route('register.post') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div id="step-1" class="step-content p-4 md:p-6">
                    ...
                    <div class="mt-4 md:mt-6">
                        ...
                        <input type="text" id="security_id" name="security_id" class="form-input" placeholder="SEC123456" value="{{ old('security_id') }}" required>
                ...
                <div id="step-3" class="step-content p-4 md:p-6 hidden">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">Add vehicle information for the Security personnel</p>
                    ...
                    <p class="text-xs text-[#706f6c] dark:text-[#A1A09A] mt-2">
                        Maximum of 3 vehicles allowed per Security personnel
                    </p>
                ...
                <div id="step-4" class="step-content p-4 md:p-6 hidden">
                    ...
                    <p class="text-xs md:text-sm text-[#706f6c] dark:text-[#A1A09A] mb-4 md:mb-6">Create login credentials for the security personnel</p>
    ...
    <div class="camera-container max-w-4xl">
        ...
        <div class="modal-footer">
            <button class="btn-camera" onclick="captureLicensePhoto()">
                <svg class="w-6 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 616 0z" /></svg>
            </button>
        </div>
    </div>
</div>
...
function showStep(step) {
    ...
    // No step indicators to update (matching student form)
...
function isValidSecurityId(securityId) {
    return securityId && securityId.trim().length >= 6;
}
...
function checkSecurityIdAvailability(securityId) {
    if (!securityId || !isValidSecurityId(securityId)) return;
    
    fetch('/check-security-id-availability', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateSecurityId() {
        ...
        if (!securityId) {
            ...
            return false;
        } else if (securityId.length < 6) {
            showFieldError('security_id', 'Security ID must contain at least 6 characters');
            return false;
        } else {
    ...
    if (securityIdInput) {
        ...
        securityIdInput.addEventListener('input', function() {
            ...
            securityIdTimeout = setTimeout(validateSecurityId, 500);
            updateButtonStates();
        });
    }
    ...
    document.getElementById('addVehicleBtn').addEventListener('click', function() {
        if (vehicleCount >= 3) {
            alert('Maximum of 3 vehicles allowed per security personnel.');
            return;
        }
        addVehicle();
    });
...
function checkPlateNumberAvailability(plateNo, vehicleIndex) {
    ...
    // This would make an AJAX call to check plate availability
    console.log('Checking plate availability for:', plateNo, 'at index:', vehicleIndex);
}
...
document.getElementById('securityRegistrationForm').addEventListener('submit', function(e) {
    e.preventDefault();
...
<div id="registrationSuccessModal" class="modal-backdrop hidden">
    <div class="modal-container">
        ...
        <div class="modal-body">
            <p>Your security registration has been submitted successfully! An administrator will review your application and notify you via email once it has been approved.</p>
