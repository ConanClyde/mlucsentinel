...
@section('title', 'Student Registration - MLUC Sentinel')
...
<div class="min-h-screen py-4 md:py-8 flex items-center justify-center">
    <div class="max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 md:mb-8 text-center">
            ...
            <p class="text-sm md:text-base text-[#706f6c] dark:text-[#A1A09A]">Register as student with vehicle information and account credentials</p>
        </div>

        <!-- Registration Form -->
        <div class="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
            <form id="studentRegistrationForm" method="POST" action="{{ route('register.post') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div id="step-1" class="step-content p-4 md:p-6">
                    ...
                    <div class="mt-4 md:mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="student_id" name="student_id" class="form-input" placeholder="221-0238-2" value="{{ old('student_id') }}" required>
                    ...
                    <div class="mt-4 md:mt-6">
                        ...
                        <input type="email" id="email" name="email" class="form-input" placeholder="john.doe@gmail.com or john.doe@student.dmmmsu.edu.ph" value="{{ old('email') }}" required>
...
let firstNameTimeout, lastNameTimeout, studentIdTimeout, emailTimeout, licenseNoTimeout;
...
// Email validation function (global scope like admin form)
function isValidEmail(email) {
    const emailRegex = /^[^\s@]+@(gmail\.com|student\.dmmmsu\.edu\.ph)$/;
    return emailRegex.test(email);
}

// Validate Student ID format (global scope like admin form)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkStudentIdAvailability(studentId) {
        if (!studentId || !isValidStudentId(studentId)) return;
        
        fetch('/check-student-id-availability', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    function validateStep4() {
        ...
        document.getElementById('submit-form').disabled = !isValid;
        return isValid;
    }
...
<div id="registrationSuccessModal" class="modal-backdrop hidden">
    <div class="modal-container">
        ...
        <div class="modal-body">
            <p>Your student registration has been submitted successfully! An administrator will review your application and notify you via email once it has been approved.</p>
