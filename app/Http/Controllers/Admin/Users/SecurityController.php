    ...
    public function update(Request $request, Security $security)
    {
        ...
        DB::transaction(function () use ($validated, $security, $request) {
            ...
            User::whereIn('user_type', ['global_administrator', 'administrator'])
                ->where('id', '!=', auth()->id())
                ->get()
                ->each(function ($user) use ($editorName, $securityName, $security) {
                    $notification = Notification::create([
                        'user_id' => $user->id,
                        'type' => 'security_updated',
                        'title' => 'Security Updated',
                        'message' => "{$editorName} updated Security {$securityName}",
                        'data' => [
    ...
    public function destroy(Security $security)
    {
        ...
        DB::transaction(function () use ($security, $editorName, $securityName) {
            ...
            User::whereIn('user_type', ['global_administrator', 'administrator'])
                ->where('id', '!=', auth()->id())
                ->get()
                ->each(function ($user) use ($editorName, $securityName) {
                    $notification = Notification::create([
                        'user_id' => $user->id,
                        'type' => 'security_deleted',
                        'title' => 'Security Removed',
                        'message' => "{$editorName} removed {$securityName}",
                        'data' => [
                            'action' => 'deleted',
                            'url' => '/users/security',
                        ],
